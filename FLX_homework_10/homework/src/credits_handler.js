function userCard(index) {
    ...
    return {
        ...
        putCredits(credits) {
            ...
            let history = {operationType: 'Received credits',
                credits: credits, operationTime:new Date().toLocaleString('en-GB')}
            defaults.historyLogs.push(history);
        },
        takeCredits(credits) {
            if (credits <= defaults.balance && credits <= defaults.transactionLimit) {
                defaults.balance -= credits;
                let history = {operationType: 'Withdrawal of credits',
                     credits: credits, operationTime: new Date().toLocaleString('en-GB')}
                defaults.historyLogs.push(history);
            } else {
        ...
        setTransactionLimit(credits) {
            ...
            let history = {operationType: 'Transaction limit change',
                credits: credits, operationTime: new Date().toLocaleString('en-GB')}
            defaults.historyLogs.push(history);
        },
        transferCredits(credits, card) {
            let tax = credits * percent / hundredPercent;
            if (credits <= defaults.balance && credits <= defaults.transactionLimit) {
